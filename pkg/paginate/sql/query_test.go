...
func TestWriteQuery(t *testing.T) {
	tests := []struct {
		...
		// Test Case 1: No filters, just pagination and sorting
		{
			...
			offset:             10,
			limit:              10,
			expectedQuery:      "SELECT * FROM users ORDER BY name ASC LIMIT $1 OFFSET $2;",
		...
		// Test Case 2: With filters and pagination
		{
			...
			offset:             0,
			limit:              5,
			expectedQuery:      "SELECT * FROM users WHERE age > $1 ORDER BY name DESC LIMIT $2 OFFSET $3;",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			query, countQuery, args := WriteQuery(tt.table, tt.filters, tt.sortColumn, tt.descending, tt.limit, tt.offset)
